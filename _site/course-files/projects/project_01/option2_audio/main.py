...
'''
    SV Note:
    1. When creating riffs, tempo is a required keyword parameter.
    2. Other keyword parameters can also be included, which are passed in via the
       loop's "start" method.
'''
...
controller = threader.SonicPiController(tempo=60, riff=riffs.controller_riff)
controller.start()

# 2. loops play continuously until they are stopped
loop_1 = threader.SonicPiLoop(controller, [riffs.riff_1a, riffs.riff_1b])
loop_2 = threader.SonicPiLoop(controller, riffs.riff_2)
loop_3 = threader.SonicPiLoop(controller, riffs.riff_2)
loop_4 = threader.SonicPiLoop(controller, riffs.riff_2)
loop_5 = threader.SonicPiLoop(controller, riffs.riff_2)

# 3. samples only play once
sample_1 = threader.SonicPiSample(controller, riffs.riff_3)
sample_2 = threader.SonicPiSample(controller, riffs.riff_3)
sample_3 = threader.SonicPiSample(controller, riffs.riff_3)
sample_4 = threader.SonicPiSample(controller, riffs.riff_3)
sample_5 = threader.SonicPiSample(controller, riffs.riff_3)
...
def make_faster():
    controller.tempo *= 0.9
    pass

def make_slower():
    controller.tempo *= 1.1
    pass
